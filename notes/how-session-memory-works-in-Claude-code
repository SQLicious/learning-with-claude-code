...
1. Claude Code's memory system is just local Markdown files. `MEMORY.md` is a one-line index that gets auto-loaded every session, and individual memory files (for example, `feedback_liberty_mutual_skip.md`) are only pulled via `Read` when the topic matches. There is no database, no API sync, and nothing uploaded to Anthropic. I can delete or edit the files by hand at any time.
...
4. The auto-pipeline flow (`Playwright fetch -> A-G scoring -> tracker TSV -> token log`) caught two independent kill signals on the Liberty Mutual req in under a minute: an expired deadline (`2026-04-03`, passed 13 days ago) and a comp ceiling (`$157K`) below my `$160K` floor.
...
- Fetched the JD via Playwright instead of WebFetch, which is mandatory per `CLAUDE.md` for offer verification.
...
- I initially assumed memory was some hosted cloud service. It took asking directly to realize it is just files on my disk that I fully control.
